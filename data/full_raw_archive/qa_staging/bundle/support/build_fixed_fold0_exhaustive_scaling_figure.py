#!/usr/bin/env python3
"""Build the locked 3x3 fixed-fold0 exhaustive CASM scaling figure.
...
No experimental result is embedded in this file.
"""
...
DEFAULT_INPUT = HERE / "input" / "fixed_fold0_exhaustive_combination_panel_scores.csv"
...
def make_figure(
    ...
    *,
    ...
    fig, axes = plt.subplots(3, 3, figsize=(17.2, 12.2), constrained_layout=False)
    ...
    for row_index, (panel, component, row_label) in enumerate(PLOT_ROWS):
        for column_index, (suffix, column_label) in enumerate(PLOT_COLUMNS):
            ...
            if show_mean_labels:
                for x, mean in zip(x_values, means_pct):
                    ax.annotate(
                        f"{mean:.3f}",
                        (x, mean),
                        xytext=(0, 7),
            ...
            ax.tick_params(axis="x", labelsize=8.0, pad=5)
            ax.tick_params(axis="y", labelsize=8.7)
            ax.grid(axis="y", color=GRID, alpha=0.65, linewidth=0.75)
    ...
    fig.suptitle(
        "CASM tuning-data scaling on fixed evaluation panels",
        x=0.055,
        y=0.987,
        ha="left",
        fontsize=17,
    ...
    fig.text(
        ...
        "the first labeled tick is Direct. Scatter replication unit = fold combination, not piece or seed.",
        ha="left",
        fontsize=8.7,
        color=MUTED,
    )
    fig.subplots_adjust(left=0.105, right=0.982, top=0.91, bottom=0.095, wspace=0.27, hspace=0.39)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--basename",
        default="casm_fixed_fold0_exhaustive_combination_scaling_3x3",
    )
