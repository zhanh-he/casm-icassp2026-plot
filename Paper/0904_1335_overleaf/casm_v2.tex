...
\begin{table}[t]
\centering
\caption{Comparison of beat-tracking post-processors across different backbone models. DBN default $bpm=[55,215]$ is for general music, while SMC contains many low BPM songs \cite{ahn2026smcblindspot}; therefore, SMC-opt $bpm=[30,300]$ can be applied. The same SMC-opt BPM has been applied in all post-processors, with other setups unchanged.}
...
% w. CASM
% & 55-215
% & \heatposbest{0.1} & \heatpos{0.5} & \heatpos{0.6}
% & \heatpos{0.5} & \heatpos{2.9} & \heatpos{5.8}
% & \heatneg{0.6} & \heatpos{0.4} & \heatpos{2.0} \\
