...
their respective beat grids. \emph{PLPDP} \cite{chiu2023localperiodicity} is
the authors' released multi-kernel implementation. We test a common-support
55--215 BPM setting and an SMC-opt variant with 30--300 BPM support; only the
tempo limits differ, while all other settings retain the published defaults.
Specifically, the 1-, 3-, and 5-s PLP curves are multiplied, the 1-s kernel's
lower limit remains 60 BPM as required to contain a complete period, and the
50-Hz sigmoid beat activation is linearly resampled to the implementation's
native 100 Hz. PLPDP is beat-only, so the unchanged Direct downbeat maxima are
snapped to its decoded beat grid, as for the other beat-only baselines.

\subsection{Frozen operating point and scale analysis}

Every CASM row in Sec.~\ref{sec:results} uses the same explicitly loaded
Frozen-4F configuration apart from the reported tempo-support comparison:
\[
\begin{split}
&F=50,\quad \theta_{\mathrm c}=0.03,\quad
(B_{\min},B_{\max})=(55,215)\ \text{(default)},\\
...
\end{split}
\]
The CASM-SMC-opt row changes only $(B_{\min},B_{\max})$ to $(30,300)$.
The remaining scalar settings were selected on the union of SMC folds
...
others are percentage-point changes. Default CASM/PLPDP use $[55,215]$ BPM;
their SMC-opt rows use $[30,300]$ BPM. DBN-SMC-opt follows
Sec.~\ref{sec:exp}; all SMC-opt rows are target-selected diagnostics.}
...
& $\mathbf{+0.1}$ & $+0.3$ & $+0.4$
& $\mathbf{+0.5}$ & $+2.9$ & $+5.9$
& $\mathbf{+0.3}$ & $\mathbf{+2.5}$ & $+3.2$ \\
\rowcolor{gray!10}
\quad w. DBN \cite{krebs2015dbn}
& $-1.0$ & $+0.7$ & $+1.3$
...
& $-1.6$ & $+0.4$ & $\mathbf{+3.5}$ \\
\rowcolor{gray!10}
\quad w. PLPDP \cite{chiu2023localperiodicity}
& $-0.1$ & $\mathbf{+0.8}$ & $+0.1$
& $-0.2$ & $+0.1$ & $-0.1$
& $-1.4$ & $-4.7$ & $+0.5$ \\
\quad w. PLPDP, SMC-opt
& $\mathbf{+0.1}$ & $+0.3$ & $+0.3$
& $+0.1$ & $+0.2$ & $+0.2$
& $-0.9$ & $-3.5$ & $-0.3$ \\
\rowcolor{gray!10}
...
SMC. The closest adaptive baseline, PLPDP, is nearly F1-neutral on GTZAN and
raises beat CMLt by 0.8 points, but on SMC its 0.5-point AMLt gain accompanies
losses of 1.4 F1 and 4.7 CMLt points. Expanding its support to 30--300 BPM
reduces the F1 and CMLt losses to 0.9 and 3.5 points, respectively, but turns
the 0.5-point AMLt gain into a 0.3-point loss. Local periodicity alone therefore
does not guarantee a favourable correction under difficult activations. The
SMC-selected DBN also changes its
trade-off when transferred back to GTZAN. We therefore do not claim that CASM
dominates DBN, PLPDP, CRF, or comb filtering. The evidence supports a reusable
operating point whose corrections depend on the input rather than a new
...
Third, the main aggregate table does not isolate the sparse graph,
strength-versus-width ambiguity gate, or the two safeguards. A direct PLPDP
comparison is now included for Beat This, but fixed-period DP, component
ablations, PLPDP on the remaining backbones, and per-corpus fallback and
meter-path acceptance rates remain decisive. Track-paired
...
leave-one-dataset-out calibration, matched tempo-support baselines, broader
PLPDP and mechanism ablations, and explicit safeguard statistics. Multi-hypothesis
